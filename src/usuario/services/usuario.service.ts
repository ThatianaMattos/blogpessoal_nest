...
export class UsuarioService {
  ...
  async update(usuario: Usuario): Promise<Usuario> {
    const usuarioUpdate: Usuario = await this.findById(usuario.id); // Função para localizar o usuario pelo ID
    const usuarioBusca = await this.findByUsuario(usuario.usuario); // Função para localizar o usuario pelo email
